...
def plot_VMC_sequential(data_file_init, data_file_alpha, exact_gs_energy, save_fig=False, fig_name="VMC_Sequential_Metrics.png"):
    ...
    1. VMC pur (Couleur 1)
    2. Optimisation avec Lanczos alpha (Couleur 2)
    Sans légende interne, pensé pour être expliqué dans le \caption LaTeX.
    """
    ...
    fig, axs = plt.subplots(2, 2, figsize=(10, 8), dpi=150) # Ajuste figsize si besoin

    # --- Définition des couleurs ---
    color_init = "#457b9d"   # Bleu gris académique pour VMC Init
    color_alpha = "#fa5703"  # Orange vif pour l'optimisation Lanczos
    color_exact = "black"    # Noir pour l'énergie exacte

    # --- Fonction utilitaire pour tracer les deux phases ---
    def plot_phases(ax, y1, y2, ylabel, is_log=False):
        # Phase 1 : VMC Init
        ax.plot(iters1, y1, color=color_init, linewidth=2, linestyle='-')
        # Phase 2 : VMC Alpha
        ax.plot(iters2_shifted, y2, color=color_alpha, linewidth=2, linestyle='-')
    ...
    # --- Top Left: Energy ---
    plot_phases(axs[0, 0], eng1, eng2, r"Energy $E$")
    axs[0, 0].axhline(y=exact_gs_energy, color=color_exact, linestyle="-.", linewidth=2, zorder=0)
    #axs[0, 0].set_title("Ground State Energy Convergence", fontsize=LABEL_FONTSIZE)

    # --- Top Right: Relative Error ---
    plot_phases(axs[0, 1], err1, err2, r"$\Delta E / |E_{\mathrm{ex}}|$", is_log=True)
    #axs[0, 1].set_title("Relative Energy Error")

    # --- Bottom Left: Variance ---
    plot_phases(axs[1, 0], var1, var2, r"$\langle H^2 \rangle - \langle H \rangle^2$", is_log=True)
    #axs[1, 0].set_title("Energy Variance")

    # --- Bottom Right: Acceptance ---
    plot_phases(axs[1, 1], acc1, acc2, "Probability")
    #axs[1, 1].set_title("MCMC Acceptance Rate")
